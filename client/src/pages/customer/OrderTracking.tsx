...
export default function OrderTracking() {
  ...
  if (!orderId) {
    return (
      <div className="mx-auto max-w-md px-5 pb-20 pt-28 text-center">
        <h1 className="font-display text-3xl font-extrabold">Track your order</h1>
        <p className="mt-2 text-brand-charcoal/60">Enter your order ID (e.g. BST-8F3K2Q).</p>
        <form
          onSubmit={(e) => {
            e.preventDefault();
        ...
        >
          <input
            ...
            className="flex-1 rounded-full border border-black/10 px-5 py-2.5 outline-none focus:border-brand-red"
          />
  ...
  if (error) {
    return (
      <div className="mx-auto max-w-md px-5 pb-20 pt-28 text-center">
        ...
        <p className="mt-4 text-brand-charcoal/70">{error}</p>
  ...
  if (!order) {
    return <p className="px-5 pt-28 text-center text-brand-charcoal/50">Loading order…</p>;
  }
    ...
    <div className="mx-auto max-w-3xl px-5 pb-20 pt-28">
      <div className="card overflow-hidden">
        <div className="bg-brand-red p-6 text-white">
          ...
          <h1 className="font-display text-3xl font-extrabold">{order.orderID}</h1>
                    ...
                    <div className="flex flex-col items-center">
                      <motion.div
                        ...
                        className={`grid h-9 w-9 place-items-center rounded-full text-sm font-bold ${
                          done ? 'bg-brand-red text-white' : 'bg-brand-cream text-brand-charcoal/40'
                        }`}
                      >
                        {done ? '✓' : i + 1}
                      </motion.div>
                      {i < FLOW.length - 1 && (
                        <div className={`h-6 w-0.5 ${i < currentIdx ? 'bg-brand-red' : 'bg-black/10'}`} />
                      )}
                    </div>
                    <span className={`font-semibold ${active ? 'text-brand-red' : done ? '' : 'text-brand-charcoal/40'}`}>
                      {STATUS_LABEL[step]}
                    </span>
                  </div>
                );
        ...
        <div className="border-t border-black/5 p-6">
          <h2 className="font-display font-bold">Order details</h2>
          <div className="mt-3 space-y-1 text-sm">
            <OrderLines items={order.items} className="text-brand-charcoal/80" />
            <hr className="my-2" />
            <div className="flex justify-between"><span className="text-brand-charcoal/60">Delivery</span><span>{GHS(order.deliveryFee)}</span></div>
            <div className="flex justify-between font-display font-extrabold">
              <span>Total</span><span className="text-brand-red">{GHS(order.total)}</span>
            </div>
          </div>
          <p className="mt-4 text-sm text-brand-charcoal/60">
            {order.deliveryMethod === 'delivery'
              ? `🚴 Delivering to ${order.zone} — ${order.deliveryAddress}`
              : '🏃 Pickup at Besties counter'}
